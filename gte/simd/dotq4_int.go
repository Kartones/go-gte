//go:build amd64

package simd

import "unsafe"

// dotQ4IntAsm computes a Q4 dot product using AVX2 integer MAC instructions
// (VPMADDUBSW+VPMADDWD), per the semantics documented on DotQ4Int.
//
//go:noescape
func dotQ4IntAsm(x unsafe.Pointer, blocks unsafe.Pointer, nBlocks int) float32

// DotQ4Int computes a Q4 dot product using integer MAC instructions.
// On amd64: AVX2 VPMADDUBSW+VPMADDWD (dynamic x quantization to int8).
//
// Per block: quantizes x to int8 on-the-fly, does integer dot with Q4 nibbles,
// then descales once. ~2 integer MAC instructions per 32 elements vs ~20 float
// instructions in DotQ4.
//
// Uses AVX2 assembly when FastPathEnabled, else a scalar Go fallback.
func DotQ4Int(x unsafe.Pointer, blocks unsafe.Pointer, nBlocks int) float32 {
	if FastPathEnabled {
		return dotQ4IntAsm(x, blocks, nBlocks)
	}
	return dotQ4IntScalar(x, blocks, nBlocks)
}
